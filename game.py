...
def findItem(mainPlayer):
    """
    Lets mainplayer find random item in long grass
    -------------
    Returns
...
def generateImage(symbol):
    """
    Returns images dependend on given symbol
...
def generateMap(length_map = 10, width_map = 10, prob_stone = .20, prob_player = .05):
    ...
    width_map: int
        width of the map
    prob_stone: float
        Chance to generate stone
    prob_player: float
        Chace to generate enemy player
    Returns
    -------
    Generated map in matrix
    """
    game_map = []
...
def findBestAction():
    """
    Looks for best action in dataset-matrix.
    Chooses action with highest profit OR 
        if player can win in lesser steps => he takes that path
    """
    global dataSet
    bestAction = 0
    bestResult = 0
...
def enemyAI(player1, pokemonPlayer1, player2, pokemonPlayer2, maxlevel, reverse = False, startAction = "", level = 1, result = 0):    
    global dataSet
    """
    Returns all possible paths and stores it in a matrix, 
    every line of the matrix stands for a possible path with the according profit => (startAction, result, winning, fatal, level) 
