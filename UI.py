...
def attackMainPlayer():
    ...
    if(counter == 6):
        playerPokemon = mainPlayer.getActivePokemon()
        enemyPokemon = pokedex.chooseRandomPokemon()        
        console.config(state=NORMAL)
        console.insert(tk.END, f"\nWauw, a {enemyPokemon.getName()} showed up.\nDo you want to fight? (active pokemon = {playerPokemon.getName()})\nType 'Fight' to fight or type 'Run' to run away")
        console.config(state=DISABLED)
        counter = 8
    elif(counter == 7):
    ...
    if(counter == 9):
        ...
        if(inputText.upper() == "ATTACK"):
            ...
            for attack in attacks:
                console.config(state=NORMAL)
                console.insert(tk.END, f"\nType '{attack.getName()}' to perform attack")
                console.config(state=DISABLED)
            counter = 11
        ...
        for pokemon in pokemonList:
            if(pokemon.getName().upper() == inputText.upper()):
                ...
                console.config(state=NORMAL)
                console.insert(tk.END, f"Your active pokemon has changed to {mainPlayer.getActivePokemon()}")
                console.insert(tk.END, f"\nPress Enter to continue")
                console.config(state=DISABLED)
                counter = 14
...
def attackEnemyPokemon():
    ...
    effect = attack.getEffect()
    PPCost = attack.getPPCost()
    print(int(enemyPokemon.getPP()) - int(PPCost))
    print(int(playerPokemon.getHP()) - int(effect))
...
def attackEnemyPlayer():
    ...
    global playerBattle
    maxlevel = 4
    resultOutput = game.enemyAI(enemyPlayer, enemyPokemon, mainPlayer, playerPokemon, maxlevel)
    ...
    for action, HP, PP in resultOutput:
        ...
        items = inventory.getItems()
        for item in items:
            if (item == "Masterbal" and bestAction == "Masterbal") or (item == "Pokebal" and bestAction == "Pokebal"):
                console.config(state=NORMAL)
                console.insert(tk.END, f"\nThe enemy catched your {playerPokemon.getName()}")
                console.insert(tk.END, "\nYou lost the game")
                console.config(state=DISABLED)
                mainPlayer.deletePokemon(playerPokemon)
                counter = 16
